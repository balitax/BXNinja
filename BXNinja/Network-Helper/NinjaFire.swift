...
open class DOTFire {
    ...
    public static let shared = DOTFire()
    ...
    func request(_ url: String, method: HTTPMethod, parameters: Parameters = Parameters(), encoding: ParameterEncoding, headers: HTTPHeaders = HTTPHeaders(), completion: @escaping(_ response: Data) -> (), failure: @escaping(_ error: DOTError)-> ()) {
        ...
        manager.request(url, method: method, parameters: parameters, encoding: encoding, headers: headers).responseString(queue: DispatchQueue.main, encoding: String.Encoding.utf8) { responseData in
            ...
            if statusData == DOTHTTPStatusCode.responseSuccess.rawValue {
                ...
                if let errorData = responseData.data {
                    ...
                    guard let errorCallback = try? decoder.decode(DOTError.self, from: errorData) else {
    ...
    func upload(_ url: String, anyImage: [UIImage], anyParameters: [String: String], headers: HTTPHeaders, completion: @escaping(_ data: Data) -> (), failure: @escaping(_ error: DOTError) -> ()) {
